...
if __name__ == '__main__':
    ...
    checkpoint_path = './segnext_mscan_b_20230227-3ab7d230.pth'
    checkpoint_3channel = CheckpointLoader.load_checkpoint(checkpoint_path, map_location='cpu')
    save_path_10channel = './segnext_mscan_b_10chan.pth'
    ...
    # 保存一个中间权重，防止覆盖啥的
    cp_back_path = './segnext_mscan_l_3chan.pth'
    ...
    with torch.no_grad():
        ...
        checkpoint_10channel[weight_name][:, 4] = checkpoint_3channel[weight_name][:, 1]
        checkpoint_10channel[weight_name][:, 5] = checkpoint_3channel[weight_name][:, 0]
        checkpoint_10channel[weight_name][:, 6] = checkpoint_3channel[weight_name][:, 2]
        checkpoint_10channel[weight_name][:, 7] = checkpoint_3channel[weight_name][:, 1]
        checkpoint_10channel[weight_name][:, 8] = checkpoint_3channel[weight_name][:, 0]
        checkpoint_10channel[weight_name][:, 9] = checkpoint_3channel[weight_name][:, 2]
